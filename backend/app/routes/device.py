...
from app import db
from app.models import DeviceIdentity, DeviceSession, DeviceKeyRotation
...
@device_bp.route('/devices/key-rotations', methods=['GET'])
@token_required
def get_all_rotations(current_user):
    """Get all key rotations"""
    rotations = DeviceKeyRotation.query.order_by(DeviceKeyRotation.rotated_at.desc()).all()
    return jsonify({
        'success': True,
        'message': 'Key rotations retrieved successfully',
        'data': [DeviceKeyRotationOut.model_validate(r).model_dump() for r in rotations]
    })


@device_bp.route('/devices/key-rotations/machine/<int:machine_id>', methods=['GET'])
@token_required
def get_rotations_by_machine(current_user, machine_id):
    """Get key rotations for a machine"""
    rotations = DeviceKeyRotation.query.filter_by(machine_id=machine_id).order_by(DeviceKeyRotation.rotated_at.desc()).all()
    return jsonify({
        'success': True,
        'message': 'Key rotations retrieved successfully',
        'data': [DeviceKeyRotationOut.model_validate(r).model_dump() for r in rotations]
...
@device_bp.route('/devices/key-rotations', methods=['POST'])
@token_required
def create_rotation(current_user):
    ...
    try:
        ...
        data = DeviceKeyRotationCreate(**json_data)
        new_rotation = DeviceKeyRotation(**data.model_dump())
        ...
        return jsonify({
            'success': True,
            'message': 'Key rotation created successfully',
            'data': DeviceKeyRotationOut.model_validate(new_rotation).model_dump()
        }), 201
