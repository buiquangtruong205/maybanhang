...
async def refill_slot(slot_id: int, db: AsyncSession = Depends(get_db), current_user = Depends(get_current_user)):
    """Nạp đầy hàng cho slot nhanh chóng."""
    updated = await SlotService.refill_slot(db, slot_id, user_id=current_user.id)
    if not updated:
        raise HTTPException(status_code=404, detail="Không tìm thấy vị trí (slot) này")
    return updated
